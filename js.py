...
def trackname():
        ...
        divider()
        os.system('ffmpeg -i %s -hide_banner -map 0:v -map 0:a -map 0:s -metadata title="TroopOriginals" -metadata:s:v title="TroopOriginals" -metadata:s:s title="TroopOriginals" -metadata:s:a title="TroopOriginals" -codec copy %s/thelidhu.mp4 && mv %s/thelidhu.mp4 %s'%(outputpath,OUTPUT_PATH,OUTPUT_PATH,encodespath))
